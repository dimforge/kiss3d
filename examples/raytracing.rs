...
//! web). Build with `--features raytracing` on a capable Vulkan GPU to use the
...
async fn main() {
    ...
    match raytracer.backend() {
        RayBackend::Compute => println!("Path tracer backend: compute (BVH)"),
        #[cfg(feature = "raytracing")]
        RayBackend::HardwareRayQuery => println!("Path tracer backend: hardware ray queries"),
    }
